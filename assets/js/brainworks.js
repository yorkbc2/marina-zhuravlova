...
(function(w, d, $, ajax) {
    ...
    $(function() {
        ...
        createTabs(".tabs");
        $(".load-reviews").each(function(index, el) {
    ...
    var setLoadReviewsAction = function setLoadReviewsAction($el) {
        if (!$el) return;
        var row = $("<div />").addClass("row");
        var url = "/wp-json/wp/v2/reviews?per_page=3&page=", mediaUrl = "/wp-json/wp/v2/media?include=";
        $el.on("click", function(e) {
            $el.prop("disabled", true);
            $.get(url + REVIEWS_PAGE).done(function(data) {
