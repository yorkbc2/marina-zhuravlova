    ...
    function bw_packages() {
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                ...
                $list .= '</ul>';
                $ohead .= '<th width="25%" class="'. ($is_special ? 'special' : '') .'">'. get_the_title() .'</th>';
                $obody .= '
                    <td width="25%" class="'. ($is_special ? 'special' : '') .'">
                        <h3 class="table-mobile-header '. ($is_special ? 'table-mobile-header--special' : '') .'">
                        '.$post->post_title.'
                        </h3>
                        '.$list.'
                        <div class="sp-sm-4"></div>
                        <a href="#" target="_blank" class="button-medium pckg-'. $post->post_name .'">
                            Get price
                        </a>
                    </td>
                ';                 
